...
const CartUI = {
  ...
  updateShippingBar: (cart) => {
    ...
    const msg     = bar.querySelector('[data-shipping-msg]');
    ...
    if (msg) {
      if (remaining <= 0) {
        msg.textContent = window.ShopifyCart.settings.freeShippingAchievedText;
        msg.classList.add('free-shipping-bar__message--achieved');
      } else {
        const amount = `Rs.${(remaining / 100).toFixed(2)}`;
        msg.textContent = `Add ${amount} more for free shipping!`;
